...
import { Trophy, Flame, Medal, ChevronDown, Star, Target, Shield, Zap } from "lucide-react";
import { allPlayers, matchMvps, teams, tournamentMvpSlug, type Player } from "../data/stats";
...
export function Stats() {
  const [activeTab, setActiveTab] = useState<"pts" | "ast" | "reb" | "stl">("pts");
  const [openTeam, setOpenTeam] = useState<string | null>(null);

  const topLeader = (key: keyof Pick<Player, "pts" | "ast" | "reb" | "stl">, n = 5) =>
    [...allPlayers].sort((a, b) => b[key] - a[key]).slice(0, n);

  const tournamentMvp = allPlayers.find(p => p.slug === tournamentMvpSlug)!;
  const topScorer = topLeader("pts", 1)[0];
  const topAssist = topLeader("ast", 1)[0];
  const topReb    = topLeader("reb", 1)[0];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-16">
          <div className="relative overflow-hidden pb-12 border-b-[4px] border-zinc-800">
            ...
            <div className="relative z-10 text-center">
              ...
              <h1 className="font-display text-6xl md:text-[100px] uppercase leading-[0.85] tracking-[-3px] text-white mb-4">
              ...
              <div className="col-span-2 p-8 md:p-10 flex flex-col justify-center">
                ...
                <div className="grid grid-cols-4 gap-4">
                  {[
                    ...
                    { label: "SPG", val: tournamentMvp.stl },
                  ].map(s => (
                    <div key={s.label} className="border-2 border-zinc-700 bg-zinc-950 p-3 text-center">
                      <div className="font-mono text-2xl font-bold text-white">{s.val}</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={card.label} className={`border-[3px] ${card.border} bg-zinc-900 ${card.shadow} overflow-hidden`}>
                ...
                <div className="p-5">
                  <div className={`font-mono text-5xl font-bold ${card.accent} mb-2`}>{card.val}</div>
        ...
        <section className="mb-16">
          ...
          <div className="border-[3px] border-zinc-800 bg-zinc-900 overflow-hidden">
            {matchMvps.map((mvp, i) => (
              <div key={i} className="flex items-center gap-4 px-5 py-4 border-b border-zinc-800 last:border-0 hover:bg-zinc-800/40 transition-colors group">
        ...
        <section className="mb-16">
          ...
          <div className={`border-[3px] ${activeCategory.border} bg-zinc-900 overflow-hidden`}>
            ...
            {leaderboard.map((player, i) => {
              ...
              const max = leaderboard[0][activeTab];
                ...
                <div key={player.name} className="flex items-center gap-4 px-5 py-3.5 border-b border-zinc-800/60 last:border-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1.5">
                      <Link to={`/statistiche/${player.slug}`} className={`font-sans font-bold text-sm uppercase hover:text-brand-orange transition-colors ${i === 0 ? "text-white" : "text-zinc-300"}`}>{player.name}</Link>
        ...
        <section>
          <h2 className="font-display text-2xl uppercase tracking-widest text-zinc-500 mb-6 flex items-center gap-3">
            <Shield className="w-5 h-5 text-brand-yellow" /> Statistiche per Squadra
          </h2>
          <div className="space-y-3">
            {teams.map(team => {
              const roster = allPlayers.filter(p => p.team === team).sort((a, b) => b.pts - a.pts);
                ...
                <div key={team} className={`border-[3px] transition-colors ${isOpen ? "border-zinc-600" : "border-zinc-800"} bg-zinc-900 overflow-hidden`}>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <span className="font-display text-lg uppercase tracking-wide text-white group-hover:text-brand-orange transition-colors">
                        {team}
                      </span>
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="bg-zinc-950">
                                ...
                                <th className="text-center px-4 py-3 font-display text-xs uppercase tracking-widest text-brand-orange">PPG</th>
                                <th className="text-center px-4 py-3 font-display text-xs uppercase tracking-widest text-brand-blue">APG</th>
                                <th className="text-center px-4 py-3 font-display text-xs uppercase tracking-widest text-brand-yellow">RPG</th>
                                <th className="text-center px-4 py-3 font-display text-xs uppercase tracking-widest text-green-400">SPG</th>
                              </tr>
                            </thead>
                            <tbody>
                              {roster.map((player, i) => (
                                <tr key={player.name} className={`border-t border-zinc-800/50 ${i === 0 ? "bg-brand-orange/5" : "hover:bg-zinc-800/30"} transition-colors`}>
                                  ...
                                  <td className="text-center px-4 py-3 font-mono font-bold text-brand-orange">{player.pts}</td>
                                  <td className="text-center px-4 py-3 font-mono text-zinc-400">{player.ast}</td>
                                  <td className="text-center px-4 py-3 font-mono text-zinc-400">{player.reb}</td>
                                  <td className="text-center px-4 py-3 font-mono text-zinc-400">{player.stl}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
